...
def login():
    ...
    raise ValidationError('Password and Email required to login!', status_code=400) \
...
def signup():
    ...
    email, username, password, repeat_password = (
        request_data.get('email'),
        str(request_data.get('username')).strip().split(),
        request_data.get('password'),
    ...
    not_all = not password and not username and not email and not repeat_password  #pragma: no cover
    not_pw = repeat_password and username and email and not password   #pragma: no cover
    not_username = repeat_password and not username and email and password   #pragma: no cover
    not_email = repeat_password and username and not email and password   #pragma: no cover
    not_repeat = repeat_password and not username and email and password   #pragma: no cover
...
def check_missing_feilds(*args):
    not_all, not_pw, not_username, not_email, not_repeat = args  #pragma: no cover
    
    if not_all: #pragma: no cover
        raise ValidationError('Please provide required fields to login!', status_code=400)
    elif not_pw or not_repeat or not_username or not_email:  #pragma: no cover
        mapper = {
            'password': not_pw,
        ...
        for key in mapper.keys():
            return return_resp(key, mapper.get(key), 400) 
    

def return_resp(field, code):
    msg = '{} field is required!'.format(field.title()) #pragma: no cover
    return ValidationError(msg, status_code=code) #pragma: no cover
